...
# ===================== 访问计数内存缓冲 =====================
# 目的：减少 SQLite 写锁竞争。热门图片 60 秒内只写一次 DB，
# 中间访问在内存累加，将 N 次 UPDATE 合并为 1 次。
_access_accumulator: Dict[str, Dict[str, int]] = {}
_access_last_flush: Dict[str, float] = {}
_access_lock = threading.Lock()
_FLUSH_INTERVAL = 60  # 每个文件的最小 DB 写入间隔（秒）
...
def update_access_count(encrypted_id: str, access_type: str = 'direct_access') -> None:
    """更新访问计数（内存累积 + 定时批量写入，减少写锁竞争）

    热门图片在 _FLUSH_INTERVAL 秒内只写一次 DB，中间访问在内存累加，
    将 N 次 UPDATE 合并为 1 次，大幅降低 SQLite 写锁竞争。
    ...
    """
    now = time.time()
    cdn_delta = 1 if access_type == 'cdn_pull' else 0
    direct_delta = 1 if access_type == 'direct_access' else 0

    with _access_lock:
        entry = _access_accumulator.get(encrypted_id)
        if entry is None:
            _access_accumulator[encrypted_id] = {
                "total": 1, "cdn": cdn_delta, "direct": direct_delta
            }
        else:
            entry["total"] += 1
            entry["cdn"] += cdn_delta
            entry["direct"] += direct_delta

        last = _access_last_flush.get(encrypted_id, 0)
        if now - last < _FLUSH_INTERVAL:
            return  # 冷却期内，只累积不写入

        # 到达刷新时间，取出累积值并更新时间戳
        acc = _access_accumulator.pop(encrypted_id, None)
        _access_last_flush[encrypted_id] = now

    if acc is None:
        return

    _flush_access_count(encrypted_id, acc["total"], acc["cdn"], acc["direct"])


def _flush_access_count(encrypted_id: str, total: int, cdn: int, direct: int) -> None:
    """将累积的访问计数写入数据库（内部函数，调用方不需关心）"""
    with get_connection() as conn:
        cursor = conn.cursor()
        try:
            cursor.execute('''
...
def flush_all_access_counts() -> None:
    """刷新所有内存中的访问计数到数据库（用于优雅关闭，防止丢失未写入的计数）"""
    with _access_lock:
        if not _access_accumulator:
            return
        snapshot = dict(_access_accumulator)
        _access_accumulator.clear()

    count = 0
    for encrypted_id, acc in snapshot.items():
        _flush_access_count(encrypted_id, acc["total"], acc["cdn"], acc["direct"])
        count += 1

    if count > 0:
        logger.info(f"已刷新 {count} 个文件的访问计数到数据库")
