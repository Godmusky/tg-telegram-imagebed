...
# XSS 危险标签（移除整个标签及内容）
_XSS_DANGEROUS_TAGS_RE = re.compile(
    r'<\s*(script|iframe|object|embed|form|input|button|select|textarea|style|link|meta)\b'
    r'[^>]*>.*?<\s*/\s*\1\s*>',
    re.IGNORECASE | re.DOTALL,
)
# XSS 危险自闭合标签
_XSS_SELF_CLOSING_RE = re.compile(
    r'<\s*(iframe|object|embed|input|link|meta)\b[^>]*/?\s*>',
    re.IGNORECASE,
)
# on* 事件处理器属性 (onclick, onerror, onload, onmouseover 等)
_XSS_EVENT_ATTR_RE = re.compile(
    r'\s+on\w+\s*=\s*("[^"]*"|\'[^\']*\'|\S+)',
    re.IGNORECASE,
)
# javascript: 伪协议（在 href/src 中）—— 匹配整个属性值
_XSS_JS_PROTO_RE = re.compile(
    r'''(href|src|action|formaction)\s*=\s*["']\s*javascript\s*:[^"']*["']''',
    re.IGNORECASE,
)


def _sanitize_announcement_html(content: str) -> str:
    """对公告 HTML 内容进行 XSS 消毒

    移除 <script> <iframe> <object> <embed> <form> 等危险标签，
    清除 on* 事件处理器属性，过滤 javascript: 伪协议。
    此为 defense-in-depth 后端消毒，前端另有 DOMPurify 做主要防线。
    """
    ...
    # 1. 移除危险标签（含内容）
    sanitized = _XSS_DANGEROUS_TAGS_RE.sub('', sanitized)
    sanitized = _XSS_SELF_CLOSING_RE.sub('', sanitized)

    # 2. 清除 on* 事件处理器属性
    sanitized = _XSS_EVENT_ATTR_RE.sub('', sanitized)

    # 3. 过滤 javascript: 伪协议
    sanitized = _XSS_JS_PROTO_RE.sub(r'\1="#"', sanitized)
...
def get_announcement() -> Optional[Dict[str, Any]]:
    ...
    try:
        with get_connection() as conn:
            ...
            if row:
                result = dict(row)
                # 读路径 defense-in-depth：确保返回的 HTML 是干净的
                result['content'] = _sanitize_announcement_html(result['content'] or '')
                return result
            return None
...
def update_announcement(enabled: bool, content: str) -> int:
    """更新公告"""
    try:
        # 后端 defense-in-depth：存储前消毒 HTML
        content = _sanitize_announcement_html(content)
...
# CDN 相关配置键（更新后需重置 CDN 服务缓存）
CDN_RELATED_KEYS = {
    'cdn_enabled', 'cdn_monitor_enabled', 'cloudflare_cdn_domain',
    'cloudflare_api_token', 'cloudflare_zone_id', 'enable_cache_warming',
    'cloudflare_cache_level', 'cloudflare_browser_ttl', 'cloudflare_edge_ttl',
    'enable_smart_routing', 'fallback_to_origin', 'cache_warming_delay',
    'cdn_redirect_enabled', 'cdn_redirect_max_count', 'cdn_redirect_delay',
}
...
def update_system_setting(key: str, value: str) -> bool:
    ...
    try:
        ...
        with get_connection() as conn:
            ...
            # ── 缓存失效 ──────────────────────────
            # 1) 使 SettingsManager TTL 缓存失效（避免等待最长 5 秒）
            try:
                from ..settings_manager import settings_manager
                settings_manager.invalidate(key)
            except ImportError as e:
                logger.debug(f"settings_manager 导入失败，跳过缓存失效: {e}")

            # 2) CDN 相关键触发 CDN 服务缓存重置（避免等待 1 秒 TTL）
            if key in CDN_RELATED_KEYS:
                try:
                    from ..services.cdn_service import invalidate_cdn_settings_cache
                    invalidate_cdn_settings_cache()
                except ImportError as e:
                    logger.debug(f"cdn_service 导入失败，跳过 CDN 缓存失效: {e}")
...
def update_system_settings(settings: Dict[str, str]) -> bool:
    ...
    try:
        ...
        # 2) CDN 相关键触发 CDN 服务缓存重置（避免等待 1 秒 TTL）
        if any(k in CDN_RELATED_KEYS for k in settings if k in DEFAULT_SYSTEM_SETTINGS):
            try:
                from ..services.cdn_service import invalidate_cdn_settings_cache
                invalidate_cdn_settings_cache()
            except ImportError as e:
