...
def invalidate_cdn_settings_cache() -> None:
    """重置 CDN 设置缓存，使下一次读取强制从数据库获取。

    供 database/settings.py 在更新 CDN 相关配置后调用，
    避免因 1 秒 TTL 造成管理员短暂看到旧配置。
    """
    with _cdn_settings_lock:
        _CDN_SETTINGS_CACHE["ts"] = 0.0
