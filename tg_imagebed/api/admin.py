...
- galleries: 画集访问控制（/api/admin/galleries/<id>/access, /api/admin/galleries/<id>/access-tokens, /api/admin/share-all）
...
# ===================== CSRF 保护 =====================
# 对所有 state-changing 方法（POST/PUT/DELETE/PATCH）校验 CSRF token
# 端点 /api/admin/csrf-token 自身豁免（chicken-and-egg 问题）
@admin_bp.before_request
def _csrf_protect():
    """蓝图级 CSRF 校验：校验 X-CSRF-Token 头与 csrf_token cookie 一致"""
    # 测试模式下跳过（Flask test client 不自动处理 cookie/header）
    if current_app and current_app.testing:
        return None
    if request.method in ('POST', 'PUT', 'DELETE', 'PATCH'):
        # csrf-token 端点自身豁免
        if request.path.rstrip('/') == '/api/admin/csrf-token':
            return None
        header_token = request.headers.get('X-CSRF-Token', '')
        cookie_token = request.cookies.get('csrf_token', '')
        if not header_token or not cookie_token or header_token != cookie_token:
            logger.warning(
                f"CSRF 校验失败: method={request.method} path={request.path} "
                f"ip={admin_module._get_client_ip(request)}"
            )
