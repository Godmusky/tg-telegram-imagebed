...
@admin_module.login_required
def admin_cdn_warm():
    """手动触发 CDN 预热（管理员）- 异步后台执行，避免阻塞线程池"""
    if request.method == 'OPTIONS':
    ...
    # 后台线程异步执行预热，避免阻塞 waitress 线程池（P2-4 修复）
    def _warm_worker():
        warmed = 0
        probed_cached = 0
        for encrypted_id in encrypted_ids:
        ...
        logger.info(
            f'CDN预热后台任务完成: requested={len(encrypted_ids)}, '
            f'warmed_ok={warmed}, cached_after_probe={probed_cached}'
        )

    threading.Thread(target=_warm_worker, daemon=True, name='cdn-warm-worker').start()

    return add_cache_headers(jsonify({
        'success': True,
        'status': 'accepted',
        'count': len(encrypted_ids),
        'message': f'已接受 {len(encrypted_ids)} 张图片的CDN预热任务，后台执行中',
    }), 'no-cache')
