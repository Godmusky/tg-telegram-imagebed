...
def _enforce_batch_limit() -> None:
    """当字典超过 _MAX_BATCHES 时逐出最旧的 batch（防御 DDoS）"""
    excess = len(_media_group_batches) - _MAX_BATCHES
    if excess <= 0:
        return
    sorted_keys = sorted(_media_group_batches.keys(),
                         key=lambda k: _media_group_batches[k].updated_at)
    for k in sorted_keys[:excess]:
        batch = _media_group_batches.pop(k, None)
        if batch and batch.flush_task:
            batch.flush_task.cancel()
        logger.warning(f"逐出超限 batch: chat={k[0]} group={k[1]}, items={len(batch.items) if batch else '?'}")
