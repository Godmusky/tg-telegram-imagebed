...
def _sanitize_filename(caption: str, original_ext: str) -> str | None:
...
async def myuploads_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def _handle_myuploads_page(query: CallbackQuery) -> None:
    """处理上传历史翻页"""
    from ..database import get_system_setting
...
async def _handle_confirm_delete(query: CallbackQuery) -> None:
    """处理 /delete 确认/取消回调"""
...
async def _handle_quick_delete(query: CallbackQuery) -> None:
    """处理上传成功后的快速删除按钮"""
    from ..database import get_file_info, delete_files_by_ids, get_system_setting
...
async def _handle_link_format_callback(query: CallbackQuery) -> None:
...
async def _handle_settoken_callback(query: CallbackQuery) -> None:
    """处理 /settoken 选择回调"""
...
def _is_file_owner(file_info: dict, user: User) -> bool:
...
async def mytokens_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
