...
async def _wait_for_flask_ready(timeout: float = 15.0) -> bool:
    """等待 Flask 服务器就绪（健康检查轮询），供 webhook 模式使用。

    Bot 线程在设置 webhook 前需确认 Flask 已准备就绪，
    否则 Telegram 推送的 webhook 更新会因 Flask 未就绪而丢失。
    """
    ...
    while time.time() < deadline:
        try:
            await asyncio.to_thread(urllib.request.urlopen, health_url, timeout=1.0)
            logger.info("Flask 健康检查通过，webhook 可安全设置")
            return True
        except Exception:
            await asyncio.sleep(0.5)
    logger.warning(
        f"Flask 健康检查超时 ({timeout:.0f}s)，继续设置 webhook（可能存在竞态风险）"
    )
    return False
    ...
    async def start_bot():
        ...
        while True:
            ...
            try:
                ...
                proxy_url = settings_manager.get_proxy_url()
                if proxy_url:
                ...
                if update_mode == "webhook":
                    ...
                    # 等待 Flask 就绪后再设置 webhook，避免 Telegram 推送时 Flask 未启动导致丢失更新
                    await _wait_for_flask_ready()
                ...
                log_error_with_help(
                    ...
                    f"可能原因:\n"
                    f"  - 网络连接问题\n"
                    f"  - 代理配置: {settings_manager.get_proxy_url() or '未配置'}\n"
