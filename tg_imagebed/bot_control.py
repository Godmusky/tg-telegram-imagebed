...
def get_effective_bot_token() -> Tuple[str, str]:
    ...
    # 使用 settings_manager 统一访问（DB > 环境变量）
    token = str(settings_manager.get("telegram_bot_token") or "").strip()
    if token:
        # 判断来源：优先看 DB
        source = "db"
        try:
            db_token = settings_manager._get_from_db("telegram_bot_token")
            if not (db_token and db_token.strip()):
                source = "env"
        except Exception:
            source = "env"
        with _TOKEN_CACHE_LOCK:
            _TOKEN_CACHE.update({"ts": now, "token": token, "source": source})
        return token, source
