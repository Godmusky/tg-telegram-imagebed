...
def create_app() -> Flask:
    ...
    _last_set_limit_mb: int | None = None  # 缓存避免每次请求改全局 config

    def _apply_request_limit() -> int:
        """将数据库中的上传限制同步到 Flask 请求体限制（仅值变化时更新）"""
        max_mb = _get_max_upload_mb()
        nonlocal _last_set_limit_mb
        if max_mb != _last_set_limit_mb:
            # 额外留 2MB 余量给表单字段和 multipart 边界
            app.config['MAX_CONTENT_LENGTH'] = (max_mb + 2) * 1024 * 1024
            _last_set_limit_mb = max_mb
        return max_mb
    ...
    # 全局 API 速率限制（DoS 防护）
    # 测试模式下跳过 — Flask test client 所有请求都从 127.0.0.1 发出，
    # 30/min 的限制在快速连续请求下会触发 429
    from tg_imagebed.rate_limiter import check_rate_limit
    ...
    # 初始化 CORS Origin 白名单（用于 admin_helpers 手动 CORS 头校验）
    try:
        from tg_imagebed.api.admin_helpers import _init_allowed_origins
        _init_allowed_origins(admin_origins)
    except Exception:
        pass  # 无配置时使用同源策略
...
def main():
    ...
    try:
        ...
        # 刷新内存中的访问计数到数据库
        try:
            from tg_imagebed.database import flush_all_access_counts
            flush_all_access_counts()
        except Exception:
            pass
        stop_cdn_monitor()
        release_lock()
